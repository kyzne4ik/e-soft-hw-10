...
*Ответ:* 
  Я выбиру Knex, когда мне нужно будет выполнять какие-то сложные sql-запросы, потому что он более гибкий в этом плане, так как работает напрямую с sql.
  А Prisma - выберу, когда мне нужно будет написать что-то не слишком сложное, например, какой-нибудь mvp или crud-продукт и не париться в написании sql.
...
*Ответ:* 
  он использует плейсхолдеры -> `?`, например,
  ```js
    knex.raw('SELECT * FROM users WHERE id = ?', [userId])
  ```
  - именно они помогают не вставлять пользовательский данные напрямую в строку запроса.
